...
```
/queue tree
add name="WAN-Download-1" parent=bridge1 queue=cake-download max-limit=940M
add name="WAN-Upload-1" parent=global queue=cake-upload max-limit=38M
add name="EF-Upload-1" parent=global queue=cake-upload max-limit=5M priority=1 \
    packet-mark=ef-out-wan1

# EF queue gets:
# - Fixed 5M bandwidth (never throttled)
# - Priority 1 (highest, serviced before WAN-Upload at priority 8)
# - Separate queue tree (isolated from bulk)
# - Per-interface packet-mark (avoids ambiguity with multiple EF queues)
```

## Traffic Classification

Traffic reaches EF queues via per-interface mangle rules in postrouting.

**Critical: These rules must be AFTER the DSCP SET rules** (which tag packets based on
connection marks). If placed before, the dscp match sees 0 instead of the final DSCP.

Each WAN interface needs its own rule with a unique packet-mark to avoid queue tree
ambiguity (two queues with the same parent and packet-mark cause undefined behavior):

```
/ip firewall mangle
# These must be the LAST rules in postrouting (after DSCP SET rules)
add chain=postrouting dscp=46 out-interface=ether1-WAN-Spectrum \
    action=mark-packet new-packet-mark=ef-out-spectrum comment="EF upload CAKE: Spectrum"
add chain=postrouting dscp=46 out-interface=ether2-WAN-ATT \
    action=mark-packet new-packet-mark=ef-out-att comment="EF upload CAKE: ATT"
```

Common EF traffic (DSCP 46):

- VoIP RTP streams, DNS-over-TLS, NTP, STUN/TURN
- Gaming (via GAMES connection mark)
