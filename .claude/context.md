...
- v1.42 Phase 201 Plan 201-05 is wiring WANController-level DOCSIS mode telemetry and health fields. WANController now pins restart-required DOCSIS keys, presence-based setpoint plumbing, per-WAN `self.logger` INFO emission, and RED tests for six additive `/health.wans[].upload` runtime fields.
