...
- v1.42 Phase 201 Plan 201-05 wired WANController-level DOCSIS mode telemetry and health fields. WANController now pins restart-required DOCSIS keys and per-WAN `self.logger` INFO emission; `/health.wans[].upload` exposes six additive DOCSIS runtime fields via the HealthCheckHandler rate/hysteresis builder.
