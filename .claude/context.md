...
- v1.53 Phase 238 Plan 01 adds `scripts/phase238-safe17-boundary-check.sh` as a lightweight read-only SAFE-17 controller-path boundary assertion anchored to `v1.52`. It resolves `--anchor` to a commit SHA before diffing, constrains `--out` writes to `.planning/phases/238-rtt-provenance-verification-read-only-entry-gate/evidence/`, and intentionally defers the full fail-closed verifier/narrowed allowlist to Phase 239.
