...
- v1.49 Phase 227 live capture uses separate iperf3 reflector ports for simultaneous reference arms: `phase227-capture-runbook.sh` passes `--tcp-ref-port 5202` and `--ef-ref-port 5203` to the shared capture harness so unmarked UDP, unmarked TCP, and marked-EF UDP do not contend for a single iperf3 server. The shared harness keeps `--tcp-ref-port` defaulting to `--ref-port` unless explicitly separated by the runbook/operator.
