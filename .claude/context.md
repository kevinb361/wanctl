...
- v1.52 Phase 237 Plan 04 closeout records `tests/silicom/` as ignored ephemeral HIL output while preserving the option to commit redacted operator-significant summaries under phase evidence, and refreshes SAFE-16 evidence against `v1.51` with protected controller paths plus `configs/att.yaml` still clean. Treat the SAFE JSON `dirty_tree_clean` field as protected-path scoped only; use separate `git status --short` output for overall worktree state.
