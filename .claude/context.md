...
- v1.42 Phase 201 Plan 201-07 is in progress: `scripts/deploy.sh` now invokes the Spectrum-scoped predeploy gate before `/opt/wanctl` rsync, derives `REMOTE_SSH_TARGET` from `TARGET_HOST` and `REMOTE_YAML_PATH` from `/etc/wanctl/${WAN_NAME}.yaml` when unset, fails closed on gate errors, and skips ATT/non-Spectrum deploys without inspecting Spectrum YAML.
