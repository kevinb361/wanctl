...
for run in $(seq 1 "$RUNS"); do
    ...
    if [[ "$MARKED_EF" == "1" ]]; then
        (
            ...
            method="dscp"
            clean="true"
            if ! iperf3 -c "$REF_HOST" -p "$EF_REF_PORT" -u -b "$REF_UDP_RATE" -t "$ef_duration" --dscp ef --json >"$ef_json" 2>&1; then
                if grep -qiE 'unrecognized option|unknown option|invalid option|parameter error|bad.*dscp|option.*dscp' "$ef_json"; then
                    method="tos"
                    clean="true"
                    if ! iperf3 -c "$REF_HOST" -p "$EF_REF_PORT" -u -b "$REF_UDP_RATE" -t "$ef_duration" --tos 184 --json >"$ef_json" 2>&1; then
                        if grep -qiE 'unrecognized option|unknown option|invalid option|parameter error|bad.*tos|option.*tos' "$ef_json"; then
                            method="none"
                            clean="false"
                            iperf3 -c "$REF_HOST" -p "$EF_REF_PORT" -u -b "$REF_UDP_RATE" -t "$ef_duration" --json >"$ef_json" 2>&1 || true
                        fi
                    fi
                fi
            fi
