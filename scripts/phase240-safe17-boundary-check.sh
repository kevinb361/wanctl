#!/usr/bin/env bash
#
# Phase 240 SAFE-17 v1.53 boundary check.
#
# Fail-closed verifier with four layers:
#   1. controller-path diff vs v1.52 is bounded to the Phase 239 RTT seam plus
#      Phase 240 config-validator files
#   2. Phase 240 introduced no new RTT-seam diff since the Phase 239 close anchor
...
OUT=".planning/phases/240-config-validator/evidence/safe17-boundary-240.json"
...
usage() {
    cat <<'EOF'
Usage:
  scripts/phase240-safe17-boundary-check.sh [--anchor REF] [--out PATH]
...
Precondition: run after Plan 01 edits are committed and src/wanctl is clean.
EOF
}
...
emit_evidence() {
    ...
    python3 - "$ANCHOR" "$ANCHOR_SHA" "$OUT" "$V153_ALLOWLIST_RE" "$passed" "$helper_json_path" "$PHASE239_CLOSE_ANCHOR" "$RTT_SEAM_UNCHANGED_SINCE_PHASE239" <<'PY'
import json
import subprocess
import sys
...
record = {
    ...
    "notes": "SAFE-17 v1.53 Phase 240 boundary check: union path allowlist, no RTT-seam drift since Phase 239 close, protected-body, and complete allowed-diff-shape guard anchored at v1.52.",
}
...
if [[ -n "${DISALLOWED_PATHS}" ]]; then
    ...
    echo "Allowed Phase 240 src/wanctl files: rtt_backend.py, rtt_measurement.py, check_config_validators.py, check_steering_validators.py" >&2
    printf '%s\n' "${DISALLOWED_PATHS}" >&2
    emit_evidence false -
    exit 1
fi
...
if [[ -n "${RTT_SEAM_DRIFT_PATHS}" ]]; then
    echo "SAFE-17 VIOLATION: Phase 240 introduced RTT-seam drift since Phase 239 close" >&2
