#!/usr/bin/env bash
#
# Phase 244 SAFE-17 v1.53 boundary check.
#
...
OUT=".planning/phases/244-health-payload-attribution-metadata/evidence/safe17-boundary-244.json"
ALLOWED_OUT_PREFIX=".planning/phases/244-health-payload-attribution-metadata/evidence/"
...
usage() {
    cat <<'EOF'
Usage:
  scripts/phase244-safe17-boundary-check.sh [--anchor REF] [--out PATH]
  scripts/phase244-safe17-boundary-check.sh --self-test

Options:
  --anchor REF  Git anchor for SAFE-17 comparison (default: 49fb1393)
...
Precondition: run after Phase 244 source edits are committed and src/wanctl is clean.
EOF
}
...
run_self_test() {
    ...
    local tmpdir
    tmpdir="$(mktemp -d)"
    local worktree="$tmpdir/phase244-safe17-self-test"
    local base_head
    ...
    printf '\n# phase244 self-test disallowed committed edit\n' >> "$worktree/src/wanctl/queue_controller.py"
...
shape = payload.get("shape", {})
# Phase 244 is anchored after Phase 239's RTTMeasurement.probe addition, so
...
if not failures and no_new_shape_drift:
    print("PASS allowed-shape no-new-drift relative to Phase 244 anchor")
    raise SystemExit(0)
...
if [[ -n "${DISALLOWED_PATHS}" ]]; then
    ...
    echo "Allowed Phase 244 src/wanctl files: rtt_backend.py, rtt_measurement.py, check_config_validators.py, check_steering_validators.py, fping_measurement.py, reflector_scorer.py, rtt_backend_factory.py, autorate_continuous.py, steering/daemon.py, steering/health.py, health_check.py, wan_controller.py" >&2
    printf '%s\n' "${DISALLOWED_PATHS}" >&2
    emit_evidence false -
    exit 1
fi
