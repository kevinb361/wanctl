#!/usr/bin/env bash
#
# Phase 237 SAFE-16 boundary check.
#
# Read-only git evidence for the v1.52 HIL failure-injection harness closeout boundary. The check
...
usage() {
    cat <<'EOF'
Usage:
  scripts/phase237-safe16-boundary-check.sh [--anchor REF] [--out PATH]

Options:
  --anchor REF  Git anchor for SAFE-16 comparison (default: v1.51)
...
record = {
    ...
    "notes": "SAFE-16 read-only boundary check; src/wanctl/backends/ expanded to per-file hash targets from anchor/worktree tracked-file union.",
}
...
echo "SAFE-16 boundary check passed: $OUT"
