#!/usr/bin/env bash
#
# Phase 245 SAFE-17 v1.53 boundary check.
#
...
OUT=".planning/phases/245-live-a-b-rollback-anchor/evidence/safe17-boundary-245.json"
ALLOWED_OUT_PREFIX=".planning/phases/245-live-a-b-rollback-anchor/evidence/"
V153_ALLOWLIST_RE='^src/wanctl/(rtt_backend\.py|rtt_measurement\.py|check_config_validators\.py|check_steering_validators\.py|fping_measurement\.py|reflector_scorer\.py|rtt_backend_factory\.py|autorate_continuous\.py|steering/daemon\.py|steering/health\.py|health_check\.py|wan_controller\.py)$'
...
usage() {
    cat <<'EOF'
Usage:
  scripts/phase245-safe17-boundary-check.sh [--anchor REF] [--out PATH]
  scripts/phase245-safe17-boundary-check.sh --self-test

Options:
  --anchor REF  Git anchor for SAFE-17 comparison (default: ffaa8a0e)
...
Precondition: run after Phase 245 source edits are committed and src/wanctl is clean.
EOF
}
...
record = {
    ...
    "notes": "SAFE-17 v1.53 Phase 245 boundary check: expanded path allowlist, no RTT-seam drift since Phase 239 close, fping_measurement.py and reflector_scorer.py unchanged since Phase 241 close, protected-body with the exact post-gap WANController.measure_rtt fping scorer guard exception, and complete allowed-diff-shape guard anchored at v1.52.",
}
...
run_self_test() {
    ...
    local tmpdir
    tmpdir="$(mktemp -d)"
    local worktree="$tmpdir/phase245-safe17-self-test"
    local base_head
    ...
    printf '\n# phase245 self-test disallowed committed edit\n' >> "$worktree/src/wanctl/queue_controller.py"
...
protected_body_ok_with_no_new_shape_drift() {
    local helper_json_path="$1"
    python3 - "$helper_json_path" <<'PY'
import json
import sys
...
shape = payload.get("shape", {})
# Phase 245 is anchored after Phase 239's RTTMeasurement.probe addition, so
# the phase239 helper's v1.52-only allowed-shape expectation reports the
# benign "no added qualnames" shape as a failure. Treat that exact state as
# no new shape drift while preserving all protected-body checks.
no_new_shape_drift = (
    shape.get("added_qualnames") == []
    and shape.get("removed_qualnames") == []
    and shape.get("changed_nodes") == ["unexpected added qualnames: []"]
    and shape.get("module_level_ok") is True
)
if not failures and no_new_shape_drift:
    print("PASS allowed-shape no-new-drift relative to Phase 245 anchor")
    raise SystemExit(0)
raise SystemExit(f"unexpected protected/shape failures: failures={failures!r} shape={shape!r}")
PY
}
...
if [[ -n "${DISALLOWED_PATHS}" ]]; then
    ...
    echo "Allowed Phase 245 src/wanctl files: rtt_backend.py, rtt_measurement.py, check_config_validators.py, check_steering_validators.py, fping_measurement.py, reflector_scorer.py, rtt_backend_factory.py, autorate_continuous.py, steering/daemon.py, steering/health.py, health_check.py, wan_controller.py" >&2
    printf '%s\n' "${DISALLOWED_PATHS}" >&2
    emit_evidence false -
    exit 1
fi
...
if ! python3 "$SCRIPT_DIR/phase239-protected-body-diff.py" --anchor "$ANCHOR" --json >"$HELPER_JSON" 2>"$HELPER_ERR"; then
    cat "$HELPER_ERR" >&2
    if ! protected_body_ok_with_measure_rtt_exception "$HELPER_JSON" >&2       && ! protected_body_ok_with_no_new_shape_drift "$HELPER_JSON" >&2; then
