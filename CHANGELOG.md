...
- **Canary verdict engine (AUD-v1.67-002, AUD-v1.67-003):** Fixed congestion-state ranking to use `CONGESTION_ORDER` dict (GREEN=0, YELLOW=1, SOFT_RED=2, RED=3) and fail closed on unrecognized states instead of silent `.get()` fallback that ranked unknown states below GREEN; this prevented a real congestion regression from being misclassified as KEEP. Fixed `advance_phase()` to derive the **`VERDICT_KEEP`** terminal verdict from `compute_verdict()`, refusing that transition when the objective engine disagrees. **Scope of the fix:** it covers the `VERDICT_KEEP` path only. `VERDICT_REVERT` still stores `REVERT` unconditionally even when the engine returns `ABORT` (see `AUD-v1.67-012`), `ABORTED` still stamps a hardcoded rationale without consulting the engine, and `from_dict` performs no verdict cross-check on load (see `AUD-v1.67-013`). A recorded canary artifact is therefore **not** guaranteed to agree with the objective verdict engine on those paths.
