...
- **Canary verdict consistency (AUD-v1.67-012, AUD-v1.67-013):** Established the invariant that a stored verdict may never *under-report* the objective engine. Severity is ordered `KEEP < REVERT < ABORT` via `VERDICT_SEVERITY`; storing something **more** severe stays legal (an operator may revert what the engine would keep — the safe direction), storing something less severe is refused. `advance_phase(VERDICT_REVERT)` now rejects an `ABORT` verdict and directs the caller to `ABORTED`, so a canary whose rollback failed or went unverified can no longer be recorded as a plain revert. The `ABORTED` branch derives its rationale from the engine when observations exist instead of stamping a literal. `__post_init__` enforces the same invariant whenever a terminal verdict and observations are both present, which covers `from_dict` and direct construction — so a record from an older or buggy writer, or a hand-edited one, can no longer re-enter the system carrying a verdict the engine contradicts with `sha256()` attesting to it. **Still not covered:** `AUD-v1.67-011`'s read-only checker bypasses, unchanged by this work.
- **Recommendation module read-only verification (AUD-v1.67-001):** Hardened `verify_read_only()` to detect all forbidden imports: replaced attribute-path list (`os.system`, `os.popen`) with importable module names (`os`, `subprocess`, etc.); added `ast.Call` detection for the `__import__` and `import_module` names; extracted `_scan_import_surface()` helper for testability. **Scope of the guard:** it detects static imports of the listed modules and those two dynamic-import names. It does **not** detect `pathlib` writes (`write_text`/`write_bytes`/`writelines`), `exec`/`eval`, `getattr` indirection, aliased `import_module`, `ctypes`, or `asyncio` — see `AUD-v1.67-011`. It is a regression guard, not a sandbox, and must not be read as proof that a module is inert.
