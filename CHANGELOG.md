...
- **Pending external-bridge rollout:** The repository versions of both cake-autorate state bridges include the undeployed polling/SQLite rewrite from `d6a21cf3` plus the OBS-007 history-semantics correction; the live revisions were last proven during v1.66 OBS-001 before either change. Do not ship these artifacts as a side effect of an unrelated deploy. OBS-007 remains open pending a separately approved one-WAN-at-a-time exact-hash rollout with rollback and health/history/qdisc/rate proof.
