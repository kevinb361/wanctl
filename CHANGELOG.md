...
- **Recommendation module read-only verification (AUD-v1.67-001):** Hardened `verify_read_only()` to detect all forbidden imports: replaced attribute-path list (`os.system`, `os.popen`) with importable module names (`os`, `subprocess`, etc.); added detection for dynamic-import escape hatches (`__import__`, `import_module`); extracted `_scan_import_surface()` helper for testability. Verification now fails closed on any unrecognized state or control surface.
