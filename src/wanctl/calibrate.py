...
def generate_config(result: CalibrationResult, output_path: Path) -> bool:
    ...
    connection_type, alpha_baseline = _classify_connection(result.baseline_rtt_ms)
    config = _build_config_dict(result, alpha_baseline)
    header = _build_config_header(result, connection_type)
    return _write_config_file(config, header, output_path)


def _classify_connection(baseline_rtt_ms: float) -> tuple[str, float]:
    """Classify connection type from baseline RTT. Returns (type, alpha_baseline)."""
    if baseline_rtt_ms < 15:
        return "fiber", 0.01
    if baseline_rtt_ms < 35:
        return "cable", 0.02
    return "dsl", 0.015


def _build_config_dict(result: CalibrationResult, alpha_baseline: float) -> dict:
    """Build the full config dictionary from calibration results."""
    ...
    return {
        ...
        "lock_timeout": 300,
        "timeouts": {"ssh_command": 15, "ping": 1},
        "state_file": f"/var/lib/wanctl/{result.wan_name}_state.json",
    }


def _build_config_header(result: CalibrationResult, connection_type: str) -> str:
    """Build YAML header comment with calibration metadata."""
    return f"""# wanctl Configuration - {result.wan_name}
#
...
def _write_config_file(config: dict, header: str, output_path: Path) -> bool:
    """Write config dict with header to YAML file."""
    try:
