...
class QueueController:
    ...
    def __init__(self, name: str, floor: int, ceiling: int, step_up: int, factor_down: float):
        self.name = name
    ...
    def adjust(self, baseline_rtt: float, load_rtt: float, target_delta: float, warn_delta: float) -> Tuple[str, int]:
        ...
        if self.red_streak >= 1:
            ...
            new_rate = max(new_rate, self.floor_bps)
...
class ContinuousAutoRate:
    ...
    def __init__(self, config_files: List[str], debug: bool = False):
        ...
        for config_file in config_files:
            ...
            logger.info(f"Download: {config.download_floor/1e6:.0f}-{config.download_ceiling/1e6:.0f}M, "
                       f"step={config.download_step_up/1e6:.1f}M, factor={config.download_factor_down}")
            logger.info(f"Upload: {config.upload_floor/1e6:.0f}-{config.upload_ceiling/1e6:.0f}M, "
