...
VERIFIER = ROOT / "scripts" / "phase244-safe17-boundary-check.sh"
EVIDENCE = (
    ROOT
    / ".planning"
    / "phases"
    / "244-health-payload-attribution-metadata"
    / "evidence"
    / "safe17-boundary-244.json"
)

# Phase 243 close commit. Pin the point-in-time boundary instead of HEAD so
# later phases can legitimately move the tree without invalidating this test.
PHASE_CLOSE_ANCHOR = "49fb1393"
...
def test_static_phase244_script_contract():
    text = VERIFIER.read_text()

    assert VERIFIER.exists()
    assert os.access(VERIFIER, os.X_OK)
    assert "safe17-boundary-244.json" in text
    assert ".planning/phases/244-health-payload-attribution-metadata/evidence/" in text
    assert 'ANCHOR="49fb1393"' in text
    assert "V153_ALLOWLIST_RE" in text
    assert "steering/health" in text
...
def test_fails_on_out_of_allowlist_change(detached_worktree: Path):
    ...
    target.write_text(target.read_text() + "\n# phase244 out-of-allowlist drift\n")
