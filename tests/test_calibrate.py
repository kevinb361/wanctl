...
class TestConfigGeneration:
    ...
    def test_generate_config_fiber_detection(self, tmp_path):
        ...
        # Fiber should have baseline_time_constant_sec (not alpha_baseline)
        thresholds = config["continuous_monitoring"]["thresholds"]
        assert "baseline_time_constant_sec" in thresholds
        assert "alpha_baseline" not in thresholds
        # alpha=0.01, interval=0.05 -> tc=5.0s
        assert thresholds["baseline_time_constant_sec"] == 5.0
    ...
    def test_generate_config_cable_detection(self, tmp_path):
        ...
        # Cable should have baseline_time_constant_sec (not alpha_baseline)
        thresholds = config["continuous_monitoring"]["thresholds"]
        assert "baseline_time_constant_sec" in thresholds
        assert "alpha_baseline" not in thresholds
        # alpha=0.02, interval=0.05 -> tc=2.5s
        assert thresholds["baseline_time_constant_sec"] == 2.5
    ...
    def test_generate_config_dsl_detection(self, tmp_path):
        ...
        # DSL should have baseline_time_constant_sec (not alpha_baseline)
        thresholds = config["continuous_monitoring"]["thresholds"]
        assert "baseline_time_constant_sec" in thresholds
        assert "alpha_baseline" not in thresholds
        # alpha=0.015, interval=0.05 -> tc=3.33s
        assert thresholds["baseline_time_constant_sec"] == pytest.approx(3.3, abs=0.1)
    ...
    def test_generate_config_uses_modern_time_constant_keys(self, tmp_path):
        """Test config uses baseline_time_constant_sec/load_time_constant_sec, not alpha_baseline/alpha_load."""
        result = CalibrationResult(
            wan_name="wan1",
            router_host="192.168.1.1",
            router_user="admin",
            baseline_rtt_ms=25.0,  # cable: alpha_baseline=0.02
            raw_download_mbps=300.0,
            raw_upload_mbps=30.0,
            optimal_download_mbps=270.0,
            optimal_upload_mbps=27.0,
        ...
        # Modern keys present
        assert "baseline_time_constant_sec" in thresholds
        assert "load_time_constant_sec" in thresholds

        # Legacy keys absent
        assert "alpha_baseline" not in thresholds
        assert "alpha_load" not in thresholds

        # Verify load_time_constant_sec value: interval(0.05) / alpha(0.20) = 0.25
        assert thresholds["load_time_constant_sec"] == 0.25
