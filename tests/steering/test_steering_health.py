...
class TestSteeringOperatorSummary:
    """Tests for compact steering operator summary surfaces."""
    ...
    def test_health_includes_summary_section(self):
        ...
        daemon.config.green_rtt_ms = 5.0
        daemon.config.yellow_rtt_ms = 15.0
        daemon.config.red_rtt_ms = 25.0
        daemon.config.red_samples_required = 2
        daemon.config.green_samples_required = 15
        daemon.confidence_controller = None
        daemon.alert_engine = None
        daemon.state_mgr.state = {
            "current_state": "SPECTRUM_GOOD",
            "congestion_state": "GREEN",
            "red_count": 0,
        ...
        assert summary["service"] == "steering"
        assert summary["alerts"]["status"] == "disabled"
        row = summary["rows"][0]
        assert row["name"] == "steering"
        assert row["status"] == "ok"
        assert row["state"] == "SPECTRUM_GOOD"
        assert row["congestion_state"] == "GREEN"
        assert row["wan_zone"] == "GREEN"
