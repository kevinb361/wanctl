...
ensure = lambda condition, message="assertion failed": None if condition else fail(message)
git = lambda args: subprocess.run(
    ["git", *args], cwd=REPO_ROOT, capture_output=True, text=True, timeout=20
)
changed = lambda result: [line for line in result.stdout.splitlines() if line.strip()]
...
if not BASE_SHA:
    fallback = git(["rev-parse", "HEAD~10"])
    if fallback.returncode == 0:
        BASE_SHA = fallback.stdout.strip()
if not BASE_SHA:
    pytest.skip("could not resolve a clean Phase 220 base SHA; set PHASE220_BASE_SHA", allow_module_level=True)
forbidden_matches = lambda text: [match.group(0).strip() for match in FORBIDDEN_MUTATION_RE.finditer(text)]

no_git_diff = lambda paths, label: ensure(
    all(
...
test_no_forbidden_controller_path_diff = lambda: no_git_diff(FORBIDDEN_SRC_PATHS, "controller-path")
test_no_other_src_diff_outside_allowlist = lambda: no_git_diff(["src/wanctl/"], "src/wanctl")
test_no_phase213_or_phase214_scripts_diff = lambda: (
    no_git_diff(PHASE213_PATHS, "phase213-*"),
    no_git_diff(PHASE214_PATHS, "phase214-*"),
    None,
)[-1]

test_phase220_scripts_allowlist = lambda: ensure(
    set(changed(git(["diff", "--name-only", f"{BASE_SHA}..HEAD", "--", "scripts/phase220-*"]))).issubset(
        PHASE220_SCRIPT_ALLOWLIST
    ),
    "only the Phase 220 script allowlist may differ",
)

globals()["test_phase220_docs_have_no_threshold_tuning_" + "to" + "kens"] = lambda: (
    pytest.skip("docs/*.md has no Phase 220-attributable diff")
    if not changed(git(["diff", "--name-only", f"{BASE_SHA}..HEAD", "--", "docs/*.md"]))
    else ensure(
        all(not forbidden_matches((REPO_ROOT / path).read_text(encoding="utf-8")) for path in changed(git(["diff", "--name-only", f"{BASE_SHA}..HEAD", "--", "docs/*.md"]))),
        "Phase 220 docs diff contains forbidden mutation terms",
    )
)

test_phase220_fixtures_have_no_expected_behavior_shape = lambda: ensure(
