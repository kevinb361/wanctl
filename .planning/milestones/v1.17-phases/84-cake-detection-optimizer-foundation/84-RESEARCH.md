...
- Optimal CAKE defaults (link-independent): flowmode=triple-isolate, diffserv=diffserv4, nat=yes, ack-filter=yes (mapped to "filter" in RouterOS), wash=direction-dependent (yes for upload, no for download)
- wash=no on upload queue produces WARNING severity ("sends DSCP marks to ISP that ignores them; CAKE classifies before washing so diffserv4 still works")
- wash=yes on download queue produces WARNING severity ("clears your own DSCP marks before LAN/WiFi WMM devices see them")
...
**Warning signs:** wash check reports optimal for upload but the download check also says optimal (should be opposite).
...
# CAKE classifies BEFORE washing (kernel: cake_handle_diffserv reads DSCP, washes header, returns original)
# So wash=yes + diffserv4 still classifies correctly — marks stripped from wire AFTER classification
OPTIMAL_WASH = {
    "upload": "yes",     # Strip DSCP before ISP (ISP ignores them anyway; prevents adverse treatment)
    "download": "no",    # Preserve DSCP marks for LAN devices and WiFi WMM QoS
}
```
...
3. **Queue types shared across directions**
   ...
   - Recommendation: Always check per-direction. If shared type, the wash check will flag it appropriately (wash=yes is optimal for upload but sub-optimal for download).
