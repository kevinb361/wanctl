...
- **D-09:** Primary signal that `cooldown_sec` dedup holds end-to-end is **alerts-table row count per cooldown window during a sustained episode**. For the qualifying VERIFY-01 event, count `alerts` rows on that WAN+direction over each `cooldown_sec`-sized bucket spanning the episode. Authoritative because the alerts table is the operator-visible surface ALERT-03 actually cares about. **AMENDED 2026-05-26 (codex peer review):** prior wording "row count vs episode duration" with `rule_id='congestion_flapping'` column was incorrect on two counts — (a) the `alerts` table schema has no `rule_id` column (filter on `alert_type IN ('flapping_dl','flapping_ul')` instead per RESEARCH.md storage/schema.py:70-87), (b) "vs episode duration" contradicts production `cooldown_sec` semantics (see D-11 below).
- **D-10:** Secondary cross-check is `journalctl -u wanctl@<wan>` grepped for the `Alert fired: flapping_(dl|ul)` logger over the same per-cooldown-window buckets. Independent surface; captures the literal "does not log-spam at every cycle" wording from REQUIREMENTS.md ALERT-03 — the proof point is the 50ms control loop does NOT produce one log line per cycle (which would be ~12k lines per 10min window); it produces ≤1 per cooldown window. Sanity check, not the decision-gate.
- **D-11:** **AMENDED 2026-05-26 (codex peer review, Rounds 1 + 2).** Required margin: **exactly 1 alerts row per `cooldown_sec` window during a sustained episode**, not per full oscillation episode. Per-host effective `cooldown_sec` values differ: **Spectrum=600s** (explicit override at `configs/spectrum.yaml:239`, "caps firings at ~3 per event" for a 30-min oscillation, tuned 2026-04-17 from a 16-alert log-spam incident); **ATT=300s** (no override in `configs/att.yaml`; falls through to `alerting.default_cooldown_sec` default at `src/wanctl/autorate_config.py:774`). Round-1 amendment incorrectly stated "ATT carries the same default" — Round-2 codex review verified against the live config + code and corrected this; the ALERT-03 audit reads the per-host effective value at execution time (recorded in `211-VERIFY-01-evidence/EVIDENCE.md` per task 211-02-03), not a hardcoded constant. ALERT-03's "alert-once-per-oscillation-episode" literal phrasing in REQUIREMENTS.md is correctly interpreted as "alert-once-per-`cooldown_sec`-window", since `cooldown_sec` is the production deduplication boundary. If a single cooldown-sec-sized bucket within a sustained event produces ≥2 alerts rows, that is the regression ALERT-03 protects against; the documented expected behavior — ~3 rows per 30-min Spectrum event, more rows but smaller buckets on ATT — IS the passing case. The original literal-1-per-episode wording was operator-intent-aligned but cooldown-config-misaligned; this amendment reconciles them.
- **D-12:** Failure handling: if ALERT-03 fails (≥2 rows in a single `cooldown_sec` window per D-11) but VERIFY-01 passes (peak > threshold visible), **block v1.45 milestone close** and open a follow-up phase for the cooldown regression. ALERT-03 remains a hard v1.45 requirement; the gate is just per-cooldown-window not per-episode. Do NOT roll back v1.45 entirely (v1.44 had the same dedup behavior; rolling back would not help).
...
- **ALERT-03 strict-1-row-per-`cooldown_sec`-window is the bar** (amended 2026-05-26 per codex Rounds 1+2; superseded the prior strict-1-per-episode wording that conflicted with production `cooldown_sec` semantics) — don't loosen to "any margin is fine just to make the test pass". If the alerts table shows ≥2 rows in a single cooldown-sec-sized bucket within a sustained event, that IS the regression ALERT-03 protects against. ~3 rows spread across a 30-min Spectrum episode (one per 600s bucket) is the documented passing case; ATT will show more buckets at shorter cooldown (300s default).
