...
### False Positives (Fixtures Named test\_\*)
...
| ------------------------------------------ | ----------------------------------------- | --------- | ---- | ---------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `tests/test_autorate_continuous.py`        | `test_profile_flag_accepted_by_argparse`  | 380-390   | HIGH | Catches all exceptions silently; verifies nothing about --profile flag | FIXED: tests \_parse_autorate_args() directly, asserts args.profile is True |
| `tests/test_steering_daemon.py`            | `test_profile_flag_accepted_by_argparse`  | 4699-4712 | HIGH | Catches all exceptions silently; verifies nothing about --profile flag | FIXED: tests argparse directly, asserts args.profile is True                |
| `tests/test_autorate_metrics_recording.py` | `test_no_error_when_storage_disabled`     | 329-336   | HIGH | Tests dead code path (if None is not None); no assertion               | FIXED: added assert writer is None                                          |
| `tests/test_steering_confidence.py`        | `test_recovery_in_degraded_state_dry_run` | 521-530   | HIGH | Runs 210 evaluate() cycles, never checks return value or state         | FIXED: captures return value, asserts result is None                        |
...
- **6 fixtures named test\_\*** found (naming convention issue, not a test quality bug)
