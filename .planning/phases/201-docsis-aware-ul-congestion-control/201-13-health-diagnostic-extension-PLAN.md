...
tags: [phase-201, gap-closure, health-payload, observability, reviews-low-deferred, additive]
...
must_haves:
  truths:
    ...
    - "/health.wans[0].upload exposes sustained_red_cycles as a non-negative int additive field (Plan 201-14 YAML config echo; absorbed from Plan 201-14 WARNING 4)"
    ...
    - "/health.wans[0].upload exposes anti_windup_triggers as a non-negative int counter (Plan 201-14 anti-windup engagement counter; absorbed from Plan 201-14 WARNING 6 — counter pattern is preferred over WARN-level log spam under sustained engagement, mirroring HIGH-5 cycle-fidelity counter pattern)"
    - "All seven new fields are present-and-typed in legacy (docsis_mode=false) deployments — no link-type Python branching, no docsis_mode gating on the health payload (additive is global). Plan 201-14 owns the controller-side counter increments; this plan owns serialization."
  ...
  artifacts:
    - path: src/wanctl/queue_controller.py
      provides: "Ring-buffer zone_trace + max_delay_delta_us snapshot field; get_health_data() exports all seven new fields (3 from original spec + 4 absorbed from 201-14 WARNING 4/6)"
      contains: "zone_trace"
    - path: src/wanctl/health_check.py
      provides: "Upload section serialization passes all seven new fields through"
...
**Revision absorption (WARNING 4 + WARNING 6 from plan-checker review):** This plan now also owns serialization of four control-model amendment counters that Plan 201-14 increments inside the controller:
- `headroom_exhausted_streak` (cycle counter, increments per EXHAUSTED cycle, resets on AVAILABLE)
- `sustained_red_cycles` (YAML config echo for diagnostic correlation)
- `anti_windup_cycles` (YAML config echo for diagnostic correlation)
- `anti_windup_triggers` (counter, increments each time the anti-windup helper halves the integral; replaces a WARN-level log spam approach with a queryable counter — HIGH-5 pattern)

Output: seven additive /health fields, two test classes (one per file modified), no behavior change to the controller (Plan 201-14 owns the counter increments).
...
```
This plan ADDS seven more keys to the same dict. Do NOT remove or rename existing keys.

`health_check.py:323-340` is where `direction == "upload"` extends `result` with the Phase 201 fields via `result.update({...})`. Add the seven new fields to the SAME `result.update({...})` block (additive, single statement).
...
**Plan 201-14 dependency for absorbed fields (WARNING 4 + WARNING 6):** Plan 201-14 (which lands AFTER this plan, but tests for both must coexist) initializes the four absorbed counters inside QueueController.__init__:
- `self._headroom_exhausted_streak: int = 0`
- `self._sustained_red_cycles: int = 8` (constructor param)
- `self._anti_windup_cycles: int = 60` (constructor param)
- `self._anti_windup_triggers: int = 0`

Since this plan lands BEFORE Plan 201-14, the absorbed-field serialization in Task 1 step 3 must use `getattr(self, '_attr_name', default)` to be tolerant of pre-201-14 controller state. Once 201-14 lands, the attributes are always present. Tests in Task 1 must explicitly set these attributes (or use a Plan-201-14-compatible fixture that defaults them to 0/8/60) so the test passes deterministically across the wave-9-then-wave-10 ordering.
</interfaces>

<tasks>

<task type="auto" tdd="true">
  <name>Task 1: Add zone_trace ring buffer, red_streak passthrough, max_delay_delta_us snapshot, AND four absorbed 201-14 counters to QueueController</name>
  <read_first>
    ...
    - .planning/phases/201-docsis-aware-ul-congestion-control/201-VERIFICATION.md (gap-1 missing-items list and "Anti-Patterns Found" section)
    - .planning/phases/201-docsis-aware-ul-congestion-control/201-14-control-model-amendment-PLAN.md (the four absorbed-field constructor attributes — they will be set there; this plan must tolerate their absence with getattr defaults)
  </read_first>
  ...
  <behavior>
    ...
    - **Test (absorbed from 201-14 WARNING 4):** get_health_data() returns "sustained_red_cycles" as an int. Default before 201-14 lands: 8 (via getattr fallback matching the 201-14 default).
  ...
  <action>
    1. In `QueueController.__init__()` (queue_controller.py, after line 122 where `self.floor_hit_cycles = 0` is set), add:
       ...
       NOTE: The four absorbed-field counters (`_headroom_exhausted_streak`, `_sustained_red_cycles`, `_anti_windup_cycles`, `_anti_windup_triggers`) are NOT initialized in this plan — they are owned by Plan 201-14. This plan exposes them via getattr-tolerant serialization in step 3.
    ...
    2. In `QueueController.adjust()`, AFTER the `zone = self._classify_zone_3state(...)` line (currently line 161) but BEFORE `self._compute_rate_3state(zone)`:
       ...
       ```
       Note: `self.red_streak` is already maintained inside `_classify_zone_3state` (lines 188, 198, 218, 224) — no new bookkeeping needed; we just expose it.

    3. In `QueueController.get_health_data()`, extend the returned dict with seven new keys ALONGSIDE the existing Phase 201 block (around lines 611-619). Add INSIDE the same dict literal:
       ```python
       ...
       # Absorbed from Plan 201-14 WARNING 4 (control-model amendment counters):
       ...
       "sustained_red_cycles": int(getattr(self, "_sustained_red_cycles", 8)),
    ...
    4. Add a test class `TestDocsisModeDiagnosticHealth` to `tests/test_queue_controller.py` (place after `TestDocsisModeCakeCorroborator`). It must cover all 12 behaviors above (8 original + 4 absorbed). Use the existing `_make_docsis_controller` and `_cake_snapshot` helpers if present (search the test file first); if not, mirror the constructor pattern from `TestDocsisModeIntegralClassifier`. For the four absorbed-field tests, the test may either set the attributes manually on the controller fixture (testing the serialization path) or rely on the getattr defaults (testing the fallback path). Include both styles for robustness.

    5. Hot-path budget: deque.append is O(1) and `int(...)` on an int is a no-op. The `list(self._zone_trace)` copy in get_health_data is OUTSIDE the 50ms hot path (only called by /health requests). No allocations added inside adjust() beyond the deque append. The four `getattr(self, ...)` calls in get_health_data are also outside the hot path.
  </action>
  <verify>
    ...
    <automated>grep -E '"headroom_exhausted_streak"|"sustained_red_cycles"|"anti_windup_cycles"|"anti_windup_triggers"' src/wanctl/queue_controller.py | grep -v '^#' | wc -l | grep -qE '^[4-9]$'</automated>
  </verify>
  <acceptance_criteria>
    ...
    - `grep -q '"sustained_red_cycles"' src/wanctl/queue_controller.py` (absorbed WARNING 4)
    ...
    - `.venv/bin/pytest -o addopts='' tests/test_queue_controller.py::TestDocsisModeDiagnosticHealth -v` exits 0 with >= 12 tests passing
    - `.venv/bin/pytest -o addopts='' tests/test_queue_controller.py -q` exits 0 (no regression)
  </acceptance_criteria>
  <done>
    Seven diagnostic fields (3 original + 4 absorbed from 201-14) are computed/exposed via get_health_data() with getattr-tolerant fallbacks for the absorbed fields, and covered by a green test class. No existing test in test_queue_controller.py regresses.
  </done>
</task>

<task type="auto" tdd="true">
  <name>Task 2: Pass seven new fields through /health upload payload in health_check.py</name>
  <read_first>
    ...
    - src/wanctl/queue_controller.py (verify Task 1 landed: get_health_data() now returns the seven new fields)
  </read_first>
  ...
  <behavior>
    ...
    - Test: GET /health response contains `wans[0].upload.sustained_red_cycles` as an int (absorbed WARNING 4).
    ...
    - Test: When the controller is in legacy mode (docsis_mode=false), all seven new fields are STILL present in the upload section (additive, not docsis_mode-gated).
  </behavior>
  <action>
    1. In `health_check.py`, find the `if direction == "upload":` block (around line 322) where `result.update({...})` adds the Phase 201 fields. Extend the SAME `result.update({...})` dict by adding seven keys at the end of the dict literal:
       ```python
       # Original Plan 201-13 fields:
       ...
       "sustained_red_cycles": int(qc_health.get("sustained_red_cycles", 8)),
       "anti_windup_cycles": int(qc_health.get("anti_windup_cycles", 60)),
       # Absorbed from Plan 201-14 WARNING 6 (counter > log spam):
       "anti_windup_triggers": int(qc_health.get("anti_windup_triggers", 0)),
       ```
       Do NOT move or rename any existing entry. Do NOT add a docsis_mode-gated branch — additive applies to legacy too (D-16 contract; CLAUDE.md additive-only rule).

    2. Add a test class `TestPhase201DiagnosticHealthFields` to `tests/test_health_check.py`. It must cover all 9 behaviors above. Use the existing health-builder fixture pattern from the file (read first to find the canonical fixture). Build one upload-section result for both `docsis_mode_active=true` and `docsis_mode_active=false`; assert all seven new fields are present in BOTH.
  </action>
  <verify>
    ...
    <automated>grep -nE '"max_delay_delta_us"|"red_streak"|"zone_trace"|"headroom_exhausted_streak"|"sustained_red_cycles"|"anti_windup_cycles"|"anti_windup_triggers"' src/wanctl/health_check.py | grep -v '^#' | wc -l | grep -qE '^[7-9]$|^1[0-9]$'</automated>
  </verify>
  <acceptance_criteria>
    ...
    - `grep -q '"sustained_red_cycles": int(qc_health.get("sustained_red_cycles"' src/wanctl/health_check.py` (absorbed WARNING 4)
    ...
    - `.venv/bin/pytest -o addopts='' tests/test_health_check.py::TestPhase201DiagnosticHealthFields -v` exits 0 with >= 9 tests passing
  ...
  <done>
    /health upload payload exposes all seven diagnostic fields under both docsis_mode and legacy mode; existing six Phase 201 fields unchanged; no test regression.
  </done>
</task>
...
<success_criteria>
- /health.wans[0].upload contains 13 Phase 201 fields total (6 existing + 3 original new + 4 absorbed from 201-14 WARNING 4/6)
- All field types are JSON-serializable: int, str, list[str], float, bool, None
- No behavior change in QueueController (rate decisions, zone classification, integral, CAKE alignment all unchanged; the four absorbed counters are owned by Plan 201-14, this plan only serializes them)
- Test count grows by >= 21 (>= 12 in test_queue_controller, >= 9 in test_health_check); test count does not shrink
- Hot-path slice passes
</success_criteria>

<output>
After completion, create `.planning/phases/201-docsis-aware-ul-congestion-control/201-13-SUMMARY.md` per the standard template.
</output>
</content>
</invoke>