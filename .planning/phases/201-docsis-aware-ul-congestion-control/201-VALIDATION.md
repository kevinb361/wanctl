...
> **2026-05-04 REPLAN (REVIEWS):** Wave numbers shifted per HIGH-2 (Plan 05 moves Wave 2 -> 3; Plans 06/07/08/10/11/12 shift accordingly). New Wave 0 stubs added per HIGH-5, MED-4, HIGH-6. Counter-delta verdict gate (HIGH-5) added to Plans 11+12. YAML rollback (HIGH-7) added to Plan 11. See 201-REVIEWS.md for full disposition.
...
| 201-05-T1 | 05 | 3 | VALN-06 | T-201-19,20 | WANController constructor wiring + presence flags + INFO log; SIGUSR1 unchanged | unit | `.venv/bin/pytest -o addopts='' tests/test_wan_controller.py -q -k TestSigusr1ReloadScopePhase201` | tests/test_wan_controller.py | ⬜ pending |
| 201-05-T2 | 05 | 3 | VALN-06 | T-201-18,21,22,23 | /health additive runtime-state fields; flash-wear preserved | unit | `.venv/bin/pytest -o addopts='' tests/test_wan_controller.py -q -k 'TestPhase201HealthAdditive or TestPhase201FlashWear'` | tests/test_wan_controller.py | ⬜ pending |
| 201-06-T1 | 06 | 4 | VALN-06 | T-201-24,25 | Spectrum YAML reflects R5+R3 keep + R0 strip + setpoint=12 | YAML+Config validate | `.venv/bin/python -c "from wanctl.autorate_config import Config; c = Config('configs/spectrum.yaml'); assert c.docsis_mode is True and c.setpoint_mbps == 12 and c._docsis_mode_explicit is True and c._setpoint_mbps_explicit is True"` | configs/spectrum.yaml | ⬜ pending |
| 201-06-T2 | 06 | 4 | VALN-06 | T-201-27 | Version 1.42.0 in pyproject + __init__ + Dockerfile | unit | `.venv/bin/python -c "import wanctl; assert wanctl.__version__ == '1.42.0'"` + `.venv/bin/pytest -o addopts='' tests/test_health_check.py -q` | pyproject.toml + src/wanctl/__init__.py + docker/Dockerfile | ⬜ pending |
| 201-06-T3 | 06 | 4 | VALN-06 | T-201-26,28 | CHANGELOG v1.42.0 entry + CONFIGURATION migration note | doc-grep | `grep -q 'DOCSIS-Aware UL Control Mode' docs/CONFIGURATION.md && grep -q 'systemctl restart wanctl' docs/CONFIGURATION.md && grep -q '## v1.42.0' CHANGELOG.md` | CHANGELOG.md + docs/CONFIGURATION.md | ⬜ pending |
| 201-07-T1 | 07 | 5 | VALN-06 | T-201-29,30,31,32,33,34,35 | Predeploy gate exists + path-validation + three-way exit + WR-02 closure | shell-integration | `bash -n scripts/phase201-predeploy-gate.sh && .venv/bin/pytest -o addopts='' tests/test_phase201_predeploy_gate.py -v` | scripts/phase201-predeploy-gate.sh + tests/test_phase201_predeploy_gate.py | ⬜ pending |
| 201-07-T2 | 07 | 5 | VALN-06 | T-201-29,33 | deploy.sh invokes gate before rsync, fails closed on missing/non-executable gate, AND propagates the gate's actual exit code (closes Codex 2026-05-04 fail-OPEN bug: `if ! cmd; then $?` returns 0, must use `\|\| gate_rc=$?`) | shell-syntax + grep + integration | `bash -n scripts/deploy.sh && grep -q 'phase201-predeploy-gate.sh' scripts/deploy.sh && ! grep -q 'WARNING.*skipping Phase 201 gate' scripts/deploy.sh && grep -Eq '\[\[ ! -x .*phase201-predeploy-gate.*\]\]' scripts/deploy.sh && grep -q '\|\| gate_rc=\$?' scripts/deploy.sh && ! grep -Eq 'if ! .*phase201-predeploy-gate' scripts/deploy.sh` | scripts/deploy.sh | ⬜ pending |
| 201-08-T1 | 08 | 5 | VALN-06 | T-201-36,37,38,39,40 | D-12 preflight extension + /health DOCSIS-mode probe + WR-02 | shell-integration | `bash -n scripts/phase200-saturation-canary.sh && .venv/bin/pytest -o addopts='' tests/test_phase200_canary_script.py -q` | scripts/phase200-saturation-canary.sh + tests/test_phase200_canary_script.py | ⬜ pending |
| 201-08-T2 | 08 | 5 | VALN-06 | (none — capture-shape evolution) | max_delay_delta_us captured at 1 Hz for v1.43+ replay corpus | grep + SAFE-05 | `grep -q 'max_delay_delta_us' src/wanctl/wan_controller.py && .venv/bin/pytest -o addopts='' tests/test_phase_195_replay.py::test_safe05_threshold_name_counts_are_unchanged -v` | src/wanctl/wan_controller.py | ⬜ pending |
...
| 201-04-T2-FHC | 04 | 2 | VALN-06 | T-201-12 | REVIEWS HIGH-5: floor_hit_cycles counter increment in _compute_rate_3state | unit | `.venv/bin/pytest -o addopts='' tests/test_queue_controller.py -q -k 'TestDocsisModeFloorHitCounter or TestDocsisModeAboveSetpointYellowPulldown'` | tests/test_queue_controller.py | ⬜ pending |
| 201-05-T2-FHCT | 05 | 3 | VALN-06 | T-201-18 | REVIEWS HIGH-5: floor_hit_cycles_total additive /health field | unit | `.venv/bin/pytest -o addopts='' tests/test_wan_controller.py -q -k test_floor_hit_cycles_total_runtime_field_present` | tests/test_wan_controller.py | ⬜ pending |
| 201-08-T1-FAIL | 08 | 5 | VALN-06 | T-201-37 | REVIEWS HIGH-6: fail-closed env enforcement for Phase 201 canary | shell-integration | `.venv/bin/pytest -o addopts='' tests/test_phase200_canary_script.py -q -k TestPhase201EnvFailClosed` | tests/test_phase200_canary_script.py | ⬜ pending |
| 201-08-T3 | 08 | 5 | VALN-06 | T-201-37 (extended) | REVIEWS HIGH-5 closure: canary script verdict-decision is AND-coupled on `floor_hit_cycles_total_delta_loaded_window == 0` AND `ul_floor_hits_during_load == 0`; disagreement produces FAIL with diagnostic reason; verdict.json publishes start/end/delta + `primary_gate` field; legacy single-gate `[[ snapshot == 0 ]] && write_pass_verdict` pattern removed | shell-integration + grep | `bash -n scripts/phase200-saturation-canary.sh && .venv/bin/pytest -o addopts='' tests/test_phase200_canary_script.py -q -k TestPhase201CounterDeltaVerdict && ! grep -Eq 'UL_FLOOR_HITS_DURING_LOAD.*==.*0.*&&.*write_pass_verdict' scripts/phase200-saturation-canary.sh` | scripts/phase200-saturation-canary.sh + tests/test_phase200_canary_script.py | ⬜ pending |
| 201-11-T2-DELTA | 11 | 7 | VALN-06 | T-201-52b | REVIEWS HIGH-5: counter-delta primary verdict (cycle-fidelity 50ms) | manual + jq | `jq '.floor_hit_cycles_total_delta_loaded_window == 0' canary/<TS>/verdict.json` returns true | manual capture | ⬜ pending |
| 201-11-T1-YAML | 11 | 7 | VALN-06 | T-201-52a | REVIEWS HIGH-7: pre-deploy YAML snapshot + rollback YAML restore | manual + ssh | `ssh cake-shaper "test -f /etc/wanctl/spectrum.yaml.prephase201-<TS>"` exits 0 BEFORE deploy; on rollback, `ssh cake-shaper "grep -c docsis_mode: /etc/wanctl/spectrum.yaml"` returns 0 | manual capture | ⬜ pending |
| 201-10-T1 | 10 | 6 | VALN-06 | T-201-44,45,46 | Codex stop-time review with GO verdict | doc-grep | `grep -q 'Live canary launch decision' .planning/phases/201-docsis-aware-ul-congestion-control/201-10-CODEX-STOP-TIME-REVIEW.md && grep -qE 'Verdict: (GO\\|GO WITH FOLLOW-UPS)' .planning/phases/201-docsis-aware-ul-congestion-control/201-10-CODEX-STOP-TIME-REVIEW.md` | manual capture | ⬜ pending |
