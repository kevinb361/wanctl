...
- Schema versions (like `schema_version: "1.0"` which is config format version)
  ...
  <verify>grep -rn "1\.0\.0\|1\.1\.0\|1\.0\.0-rc" --include="*.md" --include="*.py" --include="*.toml" --include="*.sh" /home/kevin/projects/wanctl | grep -v CHANGELOG | grep -v ".planning" to see remaining stale versions</verify>
...
3. `grep -rn "1\.0\.0-rc\|1\.0\.0-rc5\|1\.0\.0-rc7" /home/kevin/projects/wanctl --include="*.md" --include="*.sh" | grep -v CHANGELOG | grep -v .planning` returns empty
  </verify>
