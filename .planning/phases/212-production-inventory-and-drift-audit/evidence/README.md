...
| 2026-05-27T18:47:46Z | cake-shaper | Capture redacted deployed spectrum config snapshot; command: `sudo -n cat /etc/wanctl/spectrum.yaml` | Recursive D-08 key redaction before artifact write; no /etc/wanctl/secrets output read | `config-spectrum.redacted.yaml` | Read-only production inspection; no deploy/restart/config write/RouterOS write |
| 2026-05-27T18:47:46Z | cake-shaper | Capture redacted deployed att config snapshot; command: `sudo -n cat /etc/wanctl/att.yaml` | Recursive D-08 key redaction before artifact write; no /etc/wanctl/secrets output read | `config-att.redacted.yaml` | Read-only production inspection; no deploy/restart/config write/RouterOS write |
| 2026-05-27T18:47:46Z | cake-shaper | Capture redacted deployed steering config snapshot; command: `sudo -n cat /etc/wanctl/steering.yaml` | Recursive D-08 key redaction before artifact write; no /etc/wanctl/secrets output read | `config-steering.redacted.yaml` | Read-only production inspection; no deploy/restart/config write/RouterOS write |
| 2026-05-27T18:47:46Z | cake-shaper | Capture redacted steering persisted state snapshot; command: `sudo -n cat /var/lib/wanctl/steering_state.json` | Recursive D-08 key redaction before artifact write; no recovery/backup mutation triggered | `steering-state.redacted.json` | Read-only production inspection; no deploy/restart/config write/RouterOS write |
