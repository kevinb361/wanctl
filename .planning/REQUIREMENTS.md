...
- [ ] **REQ-003** — Both WAN paths apply the same four-class contract and unclassified traffic falls back to Best Effort; duplicate bridge application classifiers are removed only after equivalent contract coverage is proven. (milestone: v1.61)
  Partial evidence: exact symmetric import/restore and Best Effort fallback assertions in `tests/test_bridge_qos_nft.py`; `../infra-ansible/scripts/routeros-qos-contract-audit.py` mechanically reports the five live application-equivalence gaps; the approval-gated finite registry in `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/generic_rtp_canary.py` has repo CI and independent-review proof. Two generic-RTP canaries were rejected because later independent API sessions observed the enabled row after the terminal default; both exact rollbacks restored the original hash and healthy baseline. Frontier repair `c34d814` replaces post-add move/enable with candidate add-time placement, three bounded fresh-session samples, and reconnecting exact cleanup; focused `28 passed`, full infra CI `86 passed`, and independent Claude second-pass review passed. This candidate is not live-proven, reports only bounded stability, and still requires a separate post-apply contract audit. Corrected live application, per-selector traffic proof, and bridge retirement remain open behind a fresh approval gate.
