...
- [ ] **REQ-003** — Both WAN paths apply the same four-class contract and unclassified traffic falls back to Best Effort; duplicate bridge application classifiers are removed only after equivalent contract coverage is proven. (milestone: v1.61)
  Partial evidence: exact symmetric import/restore and Best Effort fallback assertions in `tests/test_bridge_qos_nft.py`; `../infra-ansible/scripts/routeros-qos-contract-audit.py` mechanically reports the five live application-equivalence gaps; the approval-gated finite registry in `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/generic_rtp_canary.py` has repo CI and independent-review proof. The first generic-RTP live apply was rejected because a fresh audit observed it after the terminal default; exact rollback restored the original hash and healthy baseline. Infra commit `f0d71e3` adds fresh-session target/order proof and cleanup, but corrected live application, per-selector traffic proof, and bridge retirement remain open.
