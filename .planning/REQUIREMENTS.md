...
- [ ] **REQ-003** — Both WAN paths apply the same four-class contract and unclassified traffic falls back to Best Effort; duplicate bridge application classifiers are removed only after equivalent contract coverage is proven. (milestone: v1.61)
  Partial evidence: exact symmetric import/restore and Best Effort fallback assertions in `tests/test_bridge_qos_nft.py`; the finite registry in `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/generic_rtp_canary.py`; and the read-only audit in `../infra-ansible/scripts/routeros-qos-contract-audit.py`. Root-cause repair commit `21187c3` fixed catch-all semantics and fails closed if no catch-all exists. Generic RTP is active, audit- and natural-traffic-proven. WireGuard is now active at #37 before catch-all #38, stable at hash `862700a2...7293`, and explicitly passed by the audit while preserving generic RTP; immediate counters were zero and no probe was generated. Evidence: `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/live-canary-result-wireguard-20260718T135914Z.md`. WireGuard natural-traffic proof, SSH, UDP 3480, NNTP, and bridge retirement remain open.
