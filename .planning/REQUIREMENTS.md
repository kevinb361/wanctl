...
- [ ] **REQ-003** — Both WAN paths apply the same four-class contract and unclassified traffic falls back to Best Effort; duplicate bridge application classifiers are removed only after equivalent contract coverage is proven. (milestone: v1.61)
  Partial evidence: exact symmetric import/restore and Best Effort fallback assertions in `tests/test_bridge_qos_nft.py`; `../infra-ansible/scripts/routeros-qos-contract-audit.py` mechanically reports the five live application-equivalence gaps; the approval-gated finite registry in `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/generic_rtp_canary.py` has repo CI and independent-review proof. Three generic-RTP canaries were rejected and exactly rolled back; the third proved API ordering non-authoritative against simultaneous CLI/export-backed evaluation order. A fourth authoritative SSH-CLI attempt exited nonzero and proved exact cleanup to its fresh baseline anchor, but the pre-repair Ansible `no_log` task discarded the sanitized helper error, so its failure stage remains unknown. Infra commits `5e3d05d` and `ea008d4` provide host-key-pinned same-session numeric choreography, reconnecting fail-closed cleanup, and credential-safe sanitized failure reporting (`36` focused tests, `94` full-suite tests, offline malformed-stderr harness, independent Claude `PASS`). Production remains at the healthy five-gap baseline. Corrected live application, per-selector traffic proof, and bridge retirement remain open and require a fresh status anchor, independent preflight, rollback packet, and explicit production approval.
