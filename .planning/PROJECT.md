...
v1.57 is the active milestone (planning). Production `cake-shaper` exposes the route-management surface in dry-run mode, but Netwatch remains the active/interim route owner. v1.56 ended `not-ready` because supported read-only RouterOS ownership inspection could not be proven from `cake-shaper` (the validated SSH key path `/etc/wanctl/ssh/router.key` was inaccessible there). v1.57 repairs that gap and reruns the blocked dry-run observation.

## Current Milestone: v1.57 Supported read-only RouterOS ownership inspection

**Goal:** Repair and prove a supported read-only RouterOS ownership-inspection path from `cake-shaper`, then rerun the v1.56 dry-run observation it blocked — validating intended wanctl route decisions against live Netwatch/default-route state, with no route ownership change.

**Target features:**
- **Validated SSH path repair** — Restore/prove access to the RouterOS inspection credential (`/etc/wanctl/ssh/router.key`) on `cake-shaper`, or establish a supported equivalent read-only path, so RouterOS Netwatch/default-route state can actually be read.
- **Read-only RouterOS ownership inspection** — Prove wanctl can read live Netwatch and default-route ownership from `cake-shaper` over the validated path, with attribution distinct from bridge/state health.
- **Dry-run observation rerun** — Re-execute the bounded dry-run that v1.56 could not complete: compare intended wanctl route decisions against live Netwatch/default-route state, no mutation.
- **Readiness packet** — End with an explicit `ready-for-approval` or `not-ready` verdict for a future active-route canary; do not start an implicit canary.
...
**Delivered:** fping profiling and canary repair work plus storage-hygiene audits. Full record: `milestones/v1.54-ROADMAP.md`, requirements archive `milestones/v1.54-REQUIREMENTS.md`.

**Outcome:** No production default flip. The operator-gated native fping canary exposed real native fping mechanics; Phases 248.2–248.4 fixed stale-window, native Spectrum CAKE parity, and startup first-sample readiness. Phase 249 found no current stable flat-gauge candidates. Phase 250 deferred CAKE tin skip-on-unchanged to v1.55 because `wanctl-history --tins` is raw-history and dropped/ECN are counter-shaped.
...
**v1.54:** shipped-with-deferral 2026-06-19; fping canary repairs landed, storage hygiene audits closed, tin sparse emission deferred to v1.55.
**v1.53:** shipped 2026-06-19; production decision `stay-on-icmplib`; future fping profiling deferred as `FPING-PROFILE-01`.

**Version:** v1.56 complete — Route Management Surface Deployment. **Production controller state:** both WANs run upstream cake-autorate with wanctl state bridges (`cake-autorate-{spectrum,att}.service` + `-state-bridge.service`, live since 2026-06-08); steering now exposes route-management health in safe/off dry-run mode on `cake-shaper`, but Netwatch remains active/interim route owner and active route mutation remains deferred until a future separately approved canary. Spectrum CAKE: member-NIC `diffserv4 wash` 550M base DL autorate / fixed 18M UL. ATT: `diffserv4 nowash` 95M base DL autorate / fixed 19M UL.
...
**Milestones:** 57 shipped, shipped-with-deferral, or closed (v1.0–v1.56).
**Active milestone:** none — v1.56 complete. Phase 218 watch is dormant (its instrumentation lives in the native controller, which no longer runs Spectrum/ATT).

**Latest:** v1.56 complete — route-management surface is deployed/exposed in safe/off dry-run mode, but canary readiness is `not-ready` until supported read-only RouterOS ownership inspection is proven. Previous: v1.55 closed with `keep-netwatch` and no active route mutation canary.
