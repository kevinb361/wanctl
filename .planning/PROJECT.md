...
**Current v1.52 status:** Phase 235 complete (2026-06-12). `silicom-bypass` operator CLI + guarded verbs, stateful fake-bpctl tests, boot baseline oneshot, standalone Silicom-only deploy mode, operator runbook, live baseline evidence, clean code review, and SAFE-16 controller-path zero-diff all verified. Phase 236 (watchdog fail-open two-mode reconciliation) is next.
...
**Active milestone:** v1.52 Silicom Bypass Operationalization — Phase 235 complete; Phase 236 next. Phase 218 watch is dormant (its instrumentation lives in the native controller, which no longer runs Spectrum/ATT).

**Latest:** v1.52 Phase 235 complete — `silicom-bypass` operator CLI + boot baseline delivered; gap closure hardened standalone deploy staging/argument handling/runbook semantics; clean code review; verification passed 15/15 with SAFE-16 controller-path zero-diff.
