...
**Milestones:** 55 shipped, shipped-with-deferral, or closed (v1.0–v1.54).
**Active milestone:** v1.55 Route Ownership / Netwatch Retirement. Phase 218 watch is dormant (its instrumentation lives in the native controller, which no longer runs Spectrum/ATT).

**Latest:** v1.55 opened — route ownership / Netwatch retirement is active, with no live route mutation authorized until explicit canary approval. Previous: v1.54 shipped-with-deferral — fping canary repair path complete, flat-gauge audit no-op, CAKE tin sparse emission deferred to future sparse-history work by consumer audit.
...
_Last updated: 2026-06-15 — Phase 240 complete and verified (13/13 must-haves): inert `measurement.backend: icmplib|fping` validation is wired into autorate and steering validators, existing deployment configs validate without migration, and the Phase 240 SAFE-17 boundary verifier passed. Next v1.53 work is Phase 241 (`fping` backend offline + reflector quality)._
