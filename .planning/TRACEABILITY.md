...
| REQ-004 | QoS-independent, new-connection steering eligibility | **OPEN** | Corrected Work-VPN canary is active: live `QOS_HIGH_ATT` connection was assured/reply-seen through the ATT public address, the guarded route consumer and EF emitter incremented, and DNS remained healthy. Full-policy convergence remains open. See `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/live-canary-result.txt`. |
...
| REQ-006 | Reversible controlled live canary | **OPEN** | First apply proved staged rollback. Corrected reapply and actual FortiVPN reconnect proved endpoint/VPN reachability, ATT NAT path, assured bidirectional `QOS_HIGH_ATT`, EF-emitter increments, ATT CAKE Voice increments, DNS health, and service health. Separately approved controlled-load/both-WAN evidence remains open. |
| SAFE-24 | Production mutation gate | **PROVEN** | Explicit approval, fresh read-only anchors, exact tokens, immediate first-attempt rollback, independent correction review, renewed approval, and bounded post-change verification were exercised. No qdisc/rate/service change or bulk load occurred; corrected Work-VPN canary is active with one verified FortiVPN composite connection at the latest capture. |
