...
| REQ-005 | Read-only effective-policy audit surface | **PROVEN** | `../infra-ansible/scripts/routeros-qos-contract-audit.py`; audit tests (`12 passed`, including multiword RouterOS parsing and offline strict CLI); policy tests (`3 passed`, including baseline/canary mode validation); infra-ansible `make ci` (`41 passed`); live-capture replay correctly returned FAIL for all five application gaps and WARN for the retained disabled broad route. |
