...
**Post-close corrective evidence (REV-001/REV-002; independently verified RESOLVED):** Spectrum proof preflight now fails before capture/traffic on degraded health or non-`diffserv4` CAKE, with generator non-invocation mechanically proven. Acceptance now requires exact 5/5/5/5 capture counts and an exact generator manifest; 1/4/6 counts and manifest drift are rejected. Focused proof tests `17 passed`; full `make ci` passed (`5784 passed`, 20 skipped, 2 deselected; coverage 90.15%). A fresh independent read-only review re-ran 12 focused tests and static checks, adversarially rejected wrong source/target/port, extra probes, non-dict manifests, and over-count captures, confirmed after-snapshot retention, and marked both findings RESOLVED with no new material finding. This post-close disposition does not alter the shipped v1.62 verdict.
