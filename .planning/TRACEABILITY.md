...
| REQ-006 | Reversible controlled live canary | **OPEN** | First apply proved staged rollback. Corrected reapply proved endpoint reachability, ATT NAT path, `QOS_HIGH_ATT`, EF-emitter increments, ATT CAKE Voice increments, DNS health, and service health. Actual VPN-session and separately approved controlled-load/both-WAN evidence remain open. |
| SAFE-24 | Production mutation gate | **PROVEN** | Explicit approval, fresh read-only anchors, exact tokens, immediate first-attempt rollback, independent correction review, renewed approval, and bounded post-change verification were exercised. No qdisc/rate/service change or bulk load occurred; corrected Work-VPN canary is active with zero composite connections at final status. |
