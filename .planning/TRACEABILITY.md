...
| REQ-004 | QoS-independent, new-connection steering eligibility | **OPEN** | The approved Work-VPN canary exposed an unguarded prerouting consumer that also matched WAN replies; staged rollback returned RouterOS to baseline. The corrected package requires `in-interface-list=!WAN`, with an audit regression, and passed independent read-only review with no blockers. See `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/live-canary-result.txt`. Reapply remains approval-gated. |
| REQ-005 | Read-only effective-policy audit surface | **PROVEN** | `../infra-ansible/scripts/routeros-qos-contract-audit.py`; audit tests (`15 passed`), policy tests (`3 passed`), canary tests (`6 passed`), full infra-ansible suite (`50 passed`), and `make ci`; final read-only canary status returned `baseline`, `changed=0`. Live-capture replay still correctly exposes all five application gaps and the retained legacy-route warning. |
| REQ-006 | Reversible controlled live canary | **OPEN** | Live apply and staged rollback executed under explicit approval. Apply reached `canary`; endpoint reachability then failed, producer rollback reached `draining`, active composite connections reached zero, cleanup returned `baseline`, and DNS/endpoints recovered. Rollback is proven; Work-VPN/CAKE/both-WAN success criteria remain unmet. |
| SAFE-24 | Production mutation gate | **PROVEN** | Explicit approval, fresh read-only anchors, exact tokens, immediate failure detection, staged rollback, zero-active-connection cleanup gate, and post-rollback verification were all exercised. No qdisc/rate/service change or bulk load occurred; final RouterOS state is baseline. |

**Summary:** REQ-001, REQ-002, and REQ-005 PROVEN; REQ-003, REQ-004, and REQ-006 OPEN; SAFE-24 PROVEN. The bounded canary failed reachability and was fully rolled back; production is back at baseline.
