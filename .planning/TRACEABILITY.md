...
| REQ-004 | QoS-independent, new-connection steering eligibility | **OPEN** | Proposal plus approval-gated Work-VPN canary in `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/`; exact address-list membership, same-chain route adjacency, DNS exclusion, and staged rollback are tested, but RouterOS has not been mutated and the disabled broad `QOS_HIGH` route remains live. |
...
| REQ-006 | Reversible controlled live canary | **OPEN** | Independently reviewed approval package, immutable capture hashes, exact rollback, verification commands, partial-failure cleanup, and conntrack drain gate now exist. No live canary or rollback drill has run; SAFE-24 requires explicit operator approval. |
| SAFE-24 | Production mutation gate | **PROVEN** | Repo-only audit, tests, policy model, canary package, and rollback material; only read-only live status/capture operations ran. No deploy, restart, RouterOS mutation, qdisc change, or controlled saturation occurred. |
