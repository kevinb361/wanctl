...
| REQ-004 | QoS-independent, new-connection steering eligibility | **OPEN** | Corrected Work-VPN `QOS_HIGH_ATT` path passed bounded both-WAN load, but preflight caught the retained broad `QOS_HIGH` adaptive route enabled during congestion. It routed internal recursive-DNS requests toward ATT and both resolvers timed out. Full-policy convergence is blocked until that selector is durably replaced or made DNS-ineligible. See `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/live-canary-result.txt`. |
...
| REQ-006 | Reversible controlled live canary | **OPEN** | First apply proved staged rollback. Corrected reapply and actual FortiVPN reconnect stayed assured/reply-seen through ATT while a 163,662,768-byte, 130.67-Mbit/s Spectrum download ran; DNS passed 40/40 on both resolvers after the legacy rule was disabled, Spectrum Bulk and ATT Voice counters increased, and services stayed active. Requirement remains open because the controller-enabled legacy rule broke DNS during preflight. |
...
**Summary:** REQ-001, REQ-002, and REQ-005 PROVEN; REQ-003, REQ-004, and REQ-006 OPEN; SAFE-24 PROVEN. The corrected bounded Work-VPN canary is active and narrowly verified; full-policy deployment and classifier retirement remain blocked.
