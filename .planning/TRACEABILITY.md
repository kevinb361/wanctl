...
| REQ-003 | Symmetric four-class enforcement and safe duplicate retirement | **OPEN** | Generic RTP and WireGuard are active/audit-/traffic-proven. WireGuard is #37 before catch-all #38, stable at hash `862700a2...7293`, and recorded `1` natural packet / `60` bytes after actual offsite-tunnel use. The reviewed SSH repair prevents shadowing by the live broad TCP/22 QOS_MEDIUM producer; packet `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/approval-packet-ssh-20260718T152547Z.md` is paused at explicit approval. SSH, UDP 3480, and NNTP remain open; no bridge classifier was removed. |
...
| REQ-005 | Read-only effective-policy audit surface | **PROVEN** | `../infra-ansible/scripts/routeros-qos-contract-audit.py`; the audit identifies the true catch-all and now rejects desired terminal selectors shadowed by earlier conflicting connection-mark producers. Focused canary/audit tests pass `61`; full infra `make ci` passes `102`; independent Claude Opus second-pass review returned `PASS`. Fresh live audit exposed exactly SSH/UDP3480/NNTP while generic RTP and WireGuard passed. |
...
**Summary:** REQ-001, REQ-002, REQ-004, REQ-005, and REQ-006 PROVEN; REQ-003 OPEN; SAFE-24 PROVEN. Generic RTP and WireGuard are live, mechanically proven, and naturally traffic-proven. Duplicate bridge-classifier retirement remains gated on the three remaining application-equivalence gaps.
