...
| REQ-003 | Symmetric four-class enforcement and safe duplicate retirement | **OPEN** | Production is at the five-gap baseline. The latest direct-SSH generic-RTP insertion succeeded at #36 immediately before true catch-all #37, then rolled back fail-closed after an audit false positive. Raw-evidence root cause and TDD repair: `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/root-cause-audit-false-positive-20260718T131244Z.md`; verifier commit `21187c3`; full infra suite `95 passed`; Claude second-pass `PASS`, zero blockers. A fresh approved generic-RTP reapply and explicit per-selector PASS evidence are next; no bridge classifier was removed. |
| REQ-004 | QoS-independent, new-connection steering eligibility | **PROVEN** | `src/wanctl/steering/daemon.py::reconcile_steering_rule`; startup-reconciliation unit tests; `docs/QOS_CLASSIFICATION_CONTRACT.md`; and live evidence in `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/live-canary-result.txt`. The broad `QOS_HIGH` route is absent; the exact Work-VPN/new-connection producer is disabled and verified under `SPECTRUM_GOOD`; both resolvers passed 50/50 fresh probes. |
| REQ-005 | Read-only effective-policy audit surface | **PROVEN** | `../infra-ansible/scripts/routeros-qos-contract-audit.py`; audit tests, policy tests, canary tests, full infra-ansible suite (`51 passed`), and `make ci`; corrected live consumer passed the guard audit while the capture still exposed all five application gaps and the retained legacy-route warning. |
| REQ-006 | Reversible controlled live canary | **PROVEN** | `../infra-ansible/artifacts/network-changes/20260717_routeros-qos-composite-policy/live-canary-result.txt`: corrected canary + actual FortiVPN reconnect + bounded Spectrum load proved simultaneous ATT Voice/Spectrum Bulk behavior; approval-gated demigration and remigration were exercised; final DNS passed 50/50 per resolver; no conntrack was cleared. |
| SAFE-24 | Production mutation gate | **PROVEN** | Explicit approval, fresh read-only anchors, exact tokens, first-attempt rollback, independent correction review, renewed approval, bounded bulk load, and post-load verification were exercised. An exact emergency disable contained the DNS-unsafe broad adaptive rule without clearing conntrack or changing qdiscs, rates, or services. |

**Summary:** REQ-001, REQ-002, REQ-004, REQ-005, and REQ-006 PROVEN; REQ-003 OPEN; SAFE-24 PROVEN. DNS-safe adaptive Work-VPN steering is live and rollback-proven. The insertion path is proven, the audit false positive is fixed and independently reviewed, and duplicate bridge-classifier retirement remains gated on a fresh approved generic-RTP reapply plus the remaining four application-equivalence gaps.
